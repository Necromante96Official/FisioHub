...
type AnalysisFinanceAttendance = {
    patientName: string;
    normalizedPatientName: string;
    status: "Pagante" | "Isento";
    horario: string;
    dateIso: string;
...
type AnalysisAgendamentoRecord = {
    nome: string;
    nomeNormalizado: string;
    dataIso: string;
    horario: string;
    procedimento: string;
    statusCategoria: "atendido" | "falta";
};

type AnalysisAgendamentoGroup = {
    nome: string;
    nomeNormalizado: string;
    records: AnalysisAgendamentoRecord[];
...
export type AnalysisReportData = {
    ...
    financePatients: number;
    financePagantes: number;
    financeIsentos: number;
    financeRevenue: number;
    schedulePatients: number;
    scheduleAttended: number;
    scheduleAbsences: number;
    scheduleTotal: number;
    highlights: string[];
};

type FinanceProcedureEntry = {
    raw: string;
    baseName: string;
    frequency: 1 | 2 | 3;
    value: number;
...
const readProcessedMeta = (): ProcessedMeta | null => {
...
const readDoneSignatures = (): Set<string> => {
...
const readEvolucoesPendingHistory = (): EvolucoesPendingBatch[] => {
...
const splitAppointmentBlocks = (lines: string[]): string[][] => {
    const blocks: string[][] = [];
    let current: string[] = [];
...
const parseFieldEntriesFromLine = (line: string): Array<[string, string]> => {
...
const parsePatientsRecords = (): PatientRecord[] => {
...
const sanitizeProcedimentosValue = (value: string): string => value.replace(/\s*observa[cç][oõ]es\s*:[\s\S]*$/i, "").trim();

const normalizeFinanceStatus = (value: string, convenio: string, fallbackStatus: string, proceduresRaw: string): "Pagante" | "Isento" => {
    const merged = `${value} ${convenio} ${fallbackStatus} ${proceduresRaw}`;
    return /isento/i.test(merged) ? "Isento" : "Pagante";
};

const splitProcedures = (value: string): string[] => value
    .split(/\n|\s*;\s*|\s*\|\s*|\s*,\s*/g)
    .map((part) => part.trim())
...
const extractFrequency = (value: string): 1 | 2 | 3 => {
    const normalized = normalizeText(value);
...
const removeFrequencySuffix = (value: string): string => value
...
const normalizeSpecialtyLabel = (value: string): string => removeFrequencySuffix(value)
    .replace(/\s{2,}/g, " ")
    .replace(/\s*[-–]\s*$/g, "")
    .trim();

const getProcedurePrice = (frequency: 1 | 2 | 3): number => {
    if (frequency === 2) return 20;
    if (frequency === 3) return 15;
    return 25;
};

const parseProcedureEntries = (value: string): FinanceProcedureEntry[] => {
...
const makeAttendanceKey = (input: { referenceDateIso: string; patientName: string; horario: string; dateIso: string; proceduresRaw: string; blockIndex: number; }): string => {
    return [
...
const buildAttendanceFromPatientRecord = (record: PatientRecord): { id: string; attendance: AnalysisFinanceAttendance; } => {
...
const groupFinancePatients = (records: AnalysisFinanceAttendance[]): AnalysisFinanceGroup[] => {
    const groups = new Map<string, AnalysisFinanceGroup>();
...
const readAgendamentosRecords = (): AnalysisAgendamentoRecord[] => {
    const batches = readEvolucoesPendingHistory();
    const records: AnalysisAgendamentoRecord[] = [];
...
const sanitizeText = (value: string): string => value.replace(/\s*observa[cç][oõ]es\s*:[\s\S]*$/i, "").trim();

const deduplicateAgendamentos = (records: AnalysisAgendamentoRecord[]): AnalysisAgendamentoRecord[] => {
    const unique = new Map<string, AnalysisAgendamentoRecord>();
...
const groupAgendamentosByPatient = (records: AnalysisAgendamentoRecord[]): AnalysisAgendamentoGroup[] => {
    const groups = new Map<string, AnalysisAgendamentoGroup>();
...
const parsePendingBatch = (lines: string[], referenceDateIso: string): AnalysisPendingRecord[] => {
    const normalizedLines = lines.map((line) => line.trim()).filter((line) => line.length > 0);
    const results: AnalysisPendingRecord[] = [];
...
const createEmptyPendingDraft = (referenceDateIso: string): { nome: string; dataIso: string; horario: string; procedimento: string; status: string; assinatura: string; nomeNormalizado: string; } => ({
    nome: "",
...
const normalizePendingDraft = (draft: { nome: string; dataIso: string; horario: string; procedimento: string; status: string; assinatura: string; nomeNormalizado: string; }): AnalysisPendingRecord | null => {
...
const normalizeProcedure = (value: string): string => {
    const clean = value.replace(/\s*observa[cç][oõ]es\s*:[\s\S]*$/i, "").replace(/\s+/g, " ").trim();
    return clean || "-";
};

const classifyStatus = (status: string): "atendido" | "falta" | null => {
...
const buildHighlights = (pendingRecords: AnalysisPendingRecord[], financeGroups: AnalysisFinanceGroup[], scheduleGroups: AnalysisAgendamentoGroup[]): string[] => {
...
const groupPendingByName = (records: AnalysisPendingRecord[]): Array<{ name: string; count: number; }> => {
...
export const buildAnalysisReportData = (): AnalysisReportData => {
...
const readPendingRecords = (): AnalysisPendingRecord[] => {
    ...
    const records: AnalysisPendingRecord[] = [];
...
export const buildAnalysisReportText = (data: AnalysisReportData): string => {
...
export const renderAnalysisReportMarkup = (data: AnalysisReportData): string => {
