export type AttendanceStatusCategory = "atendido" | "falta";
...
export const splitAppointmentBlocks = (lines: string[]): string[][] => {
    const blocks: string[][] = [];
    let current: string[] = [];
...
export const parseFieldEntriesFromLine = (line: string): Array<[string, string]> => {
...
export const extractIsoDate = (value: string): string | null => {
...
export const sanitizeProcedimentosValue = (value: string): string => value
    .replace(/\s*observa[cç][oõ]es\s*:[\s\S]*$/i, "")
    .trim();

export const classifyAttendanceStatus = (status: string): AttendanceStatusCategory | null => {
