...
import { FISIOHUB_RUNTIME_KEYS, FISIOHUB_STORAGE_KEYS, type EvolucoesPendingBatch, type PatientRecord, type ProcessedMeta } from "../domain/fisiohub-models.js";
import { readStoredStringSet, writeStoredStringSet } from "../shared/stored-string-set.js";
import { extractIsoDate as extractIsoDateShared, parseFieldEntriesFromLine as parseFieldEntriesFromLineShared, sanitizeProcedimentosValue as sanitizeProcedimentosValueShared, splitAppointmentBlocks as splitAppointmentBlocksShared } from "../domain/appointment-parser.js";
import { parseProcedureEntries, type ProcedureEntry } from "../domain/procedure-parser.js";
import { loadTemplateIntoApp } from "../infrastructure/template-loader.js";
import { APP_PATHS } from "../shared/paths.js";
import { bindAnalysisDialog, bindFisioHubStorageListener, bindHoverToasts as sharedBindHoverToasts, bindTermsDialog, showSiteNotification as sharedShowSiteNotification, startFloatingHomeHint as sharedStartFloatingHomeHint, syncFooterMetadata } from "../shared/ui-feedback.js";
...
interface FinanceAttendance {
    ...
    procedureEntries: FinanceProcedureEntry[];
    value: number;
    searchIndex: string;
    batchReferenceDateIso: string;
}

interface FinancePatientGroup {
    key: string;
    patientName: string;
    normalizedPatientName: string;
    status: FinanceStatus;
    attendances: FinanceAttendance[];
...
interface FinanceSpecialtyOccurrence {
    attendanceId: string;
    patientName: string;
    normalizedPatientName: string;
    convenio: string;
...
interface FinanceSpecialtyPatientSummary {
    patientName: string;
    normalizedPatientName: string;
    status: FinanceStatus;
...
interface FinanceSpecialtyFrequencyGroup {
    frequency: FinanceSpecialtyFrequency;
    occurrences: FinanceSpecialtyOccurrence[];
    patientSummaries: FinanceSpecialtyPatientSummary[];
    totalOccurrences: number;
    totalPatients: number;
    totalValue: number;
}

interface FinanceSpecialtyGroup {
    key: string;
    baseName: string;
    ...
    procedures: string[];
    searchIndex: string;
    byFrequency: Record<FinanceSpecialtyFrequency, FinanceSpecialtyFrequencyGroup>;
}

interface StoredFinanceBatch {
...
export class FinanceiroController {
    ...
    private readonly pageTemplate = APP_PATHS.pages.financeiro;
    ...
    private activeTab: FinanceTab = "pacientes";
    private activeSpecialtyModalFrequency: FinanceSpecialtyFrequency = 1;
    private activeSpecialtyStatusFilter: FinanceStatusFilter = "all";
    private attendanceRecords: FinanceAttendance[] = [];
    private visibleAttendanceRecords: FinanceAttendance[] = [];
    private patientGroups: FinancePatientGroup[] = [];
    private specialtyGroups: FinanceSpecialtyGroup[] = [];
    private selectedPatientKey: string | null = null;
    private selectedSpecialtyKey: string | null = null;

    async bootstrap(): Promise<void> {
        this.theme.init();
        await loadTemplateIntoApp(this.appId, this.pageTemplate);
    ...
    private bindHandlers(): void {
        ...
        const searchInput = document.getElementById("financeiroSearchInput") as HTMLInputElement | null;
        searchInput?.addEventListener("input", () => {
            this.activeSearch = this.normalizeText(searchInput.value);
            this.render();
        });

        const tabButtons = Array.from(document.querySelectorAll("[data-finance-tab]")) as HTMLButtonElement[];
        ...
        patientListIds.forEach((listId) => {
            ...
            list?.addEventListener("click", (event) => {
                ...
                const button = target.closest("button[data-patient-key]") as HTMLButtonElement | null;
                if (!button) return;
        ...
        specialtiesBody?.addEventListener("click", (event) => {
            const target = event.target as HTMLElement;
            const button = target.closest("button[data-specialty-key]") as HTMLButtonElement | null;
            if (!button) return;
        ...
        const specialtyTabs = Array.from(document.querySelectorAll("[data-specialty-modal-frequency]")) as HTMLButtonElement[];
        ...
        const specialtyStatusFilter = document.getElementById("financeiroSpecialtyStatusFilter") as HTMLSelectElement | null;
    ...
    private renderTabState(): void {
        ...
        const panels = Array.from(document.querySelectorAll("[data-finance-panel]")) as HTMLElement[];
        panels.forEach((panel) => {
            panel.classList.toggle("is-active", panel.dataset.financePanel === this.activeTab);
        });
    }

    private renderPatientColumns(): void {
    ...
    private renderPatientList(containerId: string, groups: FinancePatientGroup[], kind: "isento" | "pagante"): void {
    ...
    private renderSpecialtyTable(): void {
        const container = document.getElementById("financeiroSpecialtiesTableBody") as HTMLTableSectionElement | null;
        if (!container) return;
    ...
    private openPatientDialog(patientKey: string): void {
    ...
    private openSpecialtyDialog(specialtyKey: string): void {
        this.selectedSpecialtyKey = specialtyKey;
        this.activeSpecialtyModalFrequency = 1;
        this.activeSpecialtyStatusFilter = "all";

        const statusFilter = document.getElementById("financeiroSpecialtyStatusFilter") as HTMLSelectElement | null;
        if (statusFilter) {
    ...
    private renderSpecialtyDialog(): void {
        ...
        const modalTabs = Array.from(document.querySelectorAll("[data-specialty-modal-frequency]")) as HTMLButtonElement[];
        ...
        const modalPanels = Array.from(document.querySelectorAll("[data-specialty-modal-panel]")) as HTMLElement[];
        ...
        [1, 2, 3].forEach((frequency) => this.renderSpecialtyFrequencyPanel(group, frequency as FinanceSpecialtyFrequency));
    }

    private renderSpecialtyFrequencyPanel(group: FinanceSpecialtyGroup, frequency: FinanceSpecialtyFrequency): void {
        ...
        const summaries = frequencyGroup.patientSummaries.filter((summary) => {
            if (this.activeSpecialtyStatusFilter === "all") return true;
    ...
    private loadAttendanceRecordsFromStorage(): FinanceAttendance[] {
        ...
        const records = new Map<string, FinanceAttendance>();
    ...
    private readStoredBatches(): StoredFinanceBatch[] {
        const batches: StoredFinanceBatch[] = [];
    ...
    private parseAttendanceBatch(batch: StoredFinanceBatch): FinanceAttendance[] {
        const blocks = this.splitAppointmentBlocks(batch.lines);
        const records: FinanceAttendance[] = [];
    ...
    private groupSpecialties(records: FinanceAttendance[]): FinanceSpecialtyGroup[] {
        const groups = new Map<string, FinanceSpecialtyGroup>();

        records.forEach((attendance) => {
            attendance.procedureEntries.forEach((entry) => {
                const key = this.normalizeText(entry.baseName);
                const occurrence: FinanceSpecialtyOccurrence = {
                ...
                if (!existing) {
                    const created: FinanceSpecialtyGroup = {
                        key,
                        baseName: entry.baseName,
    ...
    private pushSpecialtyOccurrence(group: FinanceSpecialtyGroup, occurrence: FinanceSpecialtyOccurrence): void {
        ...
        const summaryMap = new Map<string, FinanceSpecialtyPatientSummary>();
    ...
    private createEmptyFrequencyGroup(frequency: FinanceSpecialtyFrequency): FinanceSpecialtyFrequencyGroup {
        return {
            frequency,
            occurrences: [],
            patientSummaries: [],
    ...
    private normalizeFinanceStatus(value: string, convenio: string, fallbackStatus: string, proceduresRaw: string): FinanceStatus {
        const merged = `${value} ${convenio} ${fallbackStatus} ${proceduresRaw}`;
        return /isento/i.test(merged) ? "Isento" : "Pagante";
    }

    private resolveGroupStatus(current: FinanceStatus, incoming: FinanceStatus): FinanceStatus {
    ...
    private splitAppointmentBlocks(lines: string[]): string[][] {
        return splitAppointmentBlocksShared(lines);
    }

    private parseFieldEntriesFromLine(line: string): Array<[string, string]> {
        return parseFieldEntriesFromLineShared(line);
    }

    private extractDateFromEntries(entries: Map<string, string>, rawText: string): string | null {
    ...
    private extractIsoDate(value: string): string | null {
        return extractIsoDateShared(value);
    }
    ...
    private sanitizeProcedimentosValue(value: string): string {
        return sanitizeProcedimentosValueShared(value);
    }

    private buildSearchIndex(fields: { patientName: string; convenio: string; proceduresRaw: string; status: string; dateLabel: string; horario: string; }): string {
    ...
    private isMissingValue(value: string | undefined): boolean {
        return !value || value.trim().length === 0 || value.trim() === "-";
    }

    private describeAttendanceProcedures(attendance: FinanceAttendance): string {
    ...
    private makeAttendanceKey(input: { referenceDateIso: string; patientName: string; horario: string; dateIso: string; proceduresRaw: string; blockIndex: number; }): string {
        return [
    ...
    private getPatientDialog(): HTMLDialogElement {
        return document.getElementById("financeiroPatientDialog") as HTMLDialogElement;
    }

    private getSpecialtyDialog(): HTMLDialogElement {
        return document.getElementById("financeiroSpecialtyDialog") as HTMLDialogElement;
    }
