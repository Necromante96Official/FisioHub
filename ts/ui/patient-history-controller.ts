...
import { FISIOHUB_STORAGE_KEYS, type PatientChangeHistoryEntry, type PatientRecord } from "../domain/fisiohub-models.js";
import { buildPatientRecordKey, formatPatientChangeSummary, parsePatientChangeHistory } from "../domain/patient-history.js";
import { loadTemplateIntoApp } from "../infrastructure/template-loader.js";
import { APP_PATHS } from "../shared/paths.js";
import { bindAnalysisDialog, bindFisioHubStorageListener, bindHoverToasts as sharedBindHoverToasts, bindTermsDialog, startFloatingHomeHint as sharedStartFloatingHomeHint, syncFooterMetadata } from "../shared/ui-feedback.js";

export class PatientHistoryController {
    private readonly appId = "app";
    private readonly pageTemplate = APP_PATHS.pages.registro;
    ...
    async bootstrap(): Promise<void> {
        ...
        this.selectedPatientKey = this.getRouteSearchParams().get("patient")?.trim() ?? "";
    ...
    private bindHandlers(): void {
        ...
        backButton?.addEventListener("click", () => {
            window.location.href = "index.html#/pacientes";
        });
    }

    private render(): void {
        const record = this.getSelectedPatientRecord();
        const historyList = document.getElementById("patientRecordHistoryList") as HTMLUListElement | null;
        if (!historyList) {
            return;
        }
    ...
    private renderHistoryList(historyList: HTMLUListElement, history: PatientChangeHistoryEntry[], hasRecord: boolean): void {
    ...
    private getRouteSearchParams(): URLSearchParams {
        const hashQuery = window.location.hash.split("?")[1] ?? "";
        return new URLSearchParams(hashQuery || window.location.search);
    }

    private getSelectedPatientRecord(): PatientRecord | null {
    ...
    private getSortedHistory(record: PatientRecord): PatientChangeHistoryEntry[] {
