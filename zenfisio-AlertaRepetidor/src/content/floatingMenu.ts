import type { ScanStatus } from "./scanner.js";
...
const readPosition = async (): Promise<FloatingPosition> => {
  try {
    const result = await chrome.storage.local.get(STORAGE_KEY);
    const saved = result[STORAGE_KEY] as Partial<FloatingPosition> | undefined;
...
const readVisible = async (): Promise<boolean> => {
  try {
...
const saveVisible = async (visible: boolean): Promise<void> => {
  try {
...
const injectFloatingStyle = (): void => {
...
const setRootPosition = (root: HTMLElement, position: FloatingPosition): void => {
  root.style.left = `${position.x}px`;
  root.style.top = `${position.y}px`;
};

const updateStatusText = (root: HTMLElement, status: ScanStatus): void => {
  const statusElement = root.querySelector<HTMLElement>(".zra-menu-status");
  const startButton = root.querySelector<HTMLButtonElement>(".zra-start");
  const stopButton = root.querySelector<HTMLButtonElement>(".zra-stop");
...
export const installFloatingMenu = async (actions: FloatingMenuActions): Promise<void> => {
  ...
  const button = root.querySelector<HTMLButtonElement>(".zra-float-button");
  const startButton = root.querySelector<HTMLButtonElement>(".zra-start");
  const stopButton = root.querySelector<HTMLButtonElement>(".zra-stop");
  if (!button || !startButton || !stopButton) return;
  ...
  button.addEventListener("pointerup", event => {
    ...
    if (!moved) root.classList.toggle("is-open");
  });
...
export const toggleFloatingMenuVisibility = async (): Promise<boolean> => {
  const root = document.getElementById(ROOT_ID);
  const nextVisible = root?.dataset.visible !== "true";

  if (root) {
    root.dataset.visible = String(nextVisible);
    if (!nextVisible) root.classList.remove("is-open");
  }
...
export const registerFloatingMenuShortcut = (): void => {
