...
export const injectHighlightStyle = (): void => {
...
export const clearHighlights = (): void => {
  document.querySelectorAll<HTMLElement>(`.${HIGHLIGHT_CLASS}`).forEach(card => {
    card.classList.remove(HIGHLIGHT_CLASS);
    card.removeAttribute(LABEL_ATTR);
  });
};

export const isCardHighlighted = (card: HTMLElement, label: string): boolean => {
  return card.classList.contains(HIGHLIGHT_CLASS) && card.getAttribute(LABEL_ATTR) === label;
};

export const highlightCard = (card: HTMLElement, label: string): void => {
  if (isCardHighlighted(card, label)) return;
...
export const highlightCardIfNeeded = (card: HTMLElement, label: string): boolean => {
  if (isCardHighlighted(card, label)) return false;
