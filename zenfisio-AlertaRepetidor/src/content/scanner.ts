...
type CalendarCard = {
  ...
  top: number;
  left: number;
  hiddenAttributes: Array<{ element: HTMLElement; name: string; value: string }>;
};

type CalendarCardCandidate = Omit<CalendarCard, "time"> & {
  time: string | null;
};
...
type DetailRead = {
  ...
  time: string | null;
  repeat: ReturnType<typeof parseRepeatProgress>;
  text: string;
};
...
export const createInitialStatus = (): ScanStatus => ({
  running: false,
  totalCards: 0,
  analyzedCards: 0,
  openedDetails: 0,
...
const waitUnlessStopped = async (milliseconds: number, controller: ScanController): Promise<boolean> => {
  const step = 50;
  let elapsed = 0;
...
const hideTooltipAttributes = (element: HTMLElement): CalendarCard["hiddenAttributes"] => {
  const hiddenAttributes: CalendarCard["hiddenAttributes"] = [];
  const targets = [element, ...Array.from(element.querySelectorAll<HTMLElement>("[title], [aria-label]"))];
...
const restoreTooltipAttributes = (cards: CalendarCard[]): void => {
...
export const collectCalendarCards = (options?: { hideTooltips?: boolean }): CalendarCard[] => {
  ...
  return uniqueElements
    .filter(isVisible)
    .map((element): CalendarCardCandidate => {
      const rect = element.getBoundingClientRect();
    ...
    .filter((card): card is CalendarCard => {
...
const closeDetails = (): void => {
  const closeButtons = Array.from(document.querySelectorAll<HTMLElement>(
    ".popover .close, .modal .close, [role='dialog'] .close, [data-dismiss='popover'], [data-bs-dismiss='popover'], [data-dismiss='modal'], [data-bs-dismiss='modal'], button[aria-label='Close'], button[aria-label='Fechar']"
  ));
...
const readOpenDetails = (): DetailRead[] => {
...
const samePatient = (cardText: string, patientName: string | null): boolean => {
...
const detailMatchesCard = (detail: DetailRead, card: CalendarCard): boolean => {
  return detail.time === card.time && samePatient(card.text, detail.patientName);
};

const applyDetailToCards = (detail: DetailRead, cards: CalendarCard[], highlighted: Set<HTMLElement>): number => {
  if (!detail.repeat || !isRenewalDue(detail.repeat)) return 0;

  const card = cards.find(candidate => detailMatchesCard(detail, candidate));
  if (!card || highlighted.has(card.element)) return 0;

  if (!detail.patientName) return 0;

  persistAndHighlight(
    card.element,
    card.time,
    detail.patientName,
    `${detail.repeat.current}/${detail.repeat.total}`
  );
  highlighted.add(card.element);
  return 1;
};

const waitForDetailsAfterClick = async (card: CalendarCard, controller: ScanController): Promise<DetailRead[]> => {
  for (let attempt = 0; attempt < 14; attempt += 1) {
    if (controller.stopRequested) return [];

    const details = readOpenDetails();
    const matchingDetails = details.filter(detail => detailMatchesCard(detail, card));
    if (matchingDetails.length > 0) return matchingDetails;
    if (details.length > 0 && attempt >= 6) return details;
    if (!await waitUnlessStopped(170, controller)) return [];
  }

  return [];
};

export const runAppointmentScan = async (
  ...
  try {
    for (let index = 0; index < cards.length; index += 1) {
      ...
      closeDetails();
      if (!await waitUnlessStopped(280, controller)) break;

      card.element.scrollIntoView({ block: "center", inline: "center", behavior: "instant" });
      if (!await waitUnlessStopped(420, controller)) break;
      ...
      for (const detail of details) {
        if (detail.repeat) foundRepeats += 1;
        applyDetailToCards(detail, cards, highlighted);
      }

      closeDetails();
      if (!await waitUnlessStopped(360, controller)) break;
