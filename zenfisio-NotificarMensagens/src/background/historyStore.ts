import { HISTORY_CONFIG, STORAGE_KEYS } from "../shared/constants.js";
import { patientKeyFromName } from "../shared/patientName.js";
import { toDateKey } from "../shared/text.js";
import type { HistoryEntry, StatusKind } from "../shared/types.js";
...
type HistoryByDate = Record<string, Record<string, HistoryDayItem>>;

const recentDispatches = new Map<string, number>();
let initialized = false;
let historyByDate: HistoryByDate = {};

const buildRecentKey = (patientKey: string, statusKind: StatusKind): string => {
  return `${statusKind}:${patientKey}`;
};

const pruneRecentDispatches = (): void => {
  const now = Date.now();
...
export const initHistoryStore = async (): Promise<void> => {
  if (initialized) {
    return;
  }
  ...
  try {
    const stored = await storageGet<Record<string, unknown>>([STORAGE_KEYS.SENT_HISTORY]);
    const value = stored[STORAGE_KEYS.SENT_HISTORY];

    if (value && typeof value === "object") {
      historyByDate = value as HistoryByDate;
    }
  } catch (error) {
...
export const evaluateDispatch = (
  patientName: string,
  statusKind: StatusKind
): { canSend: boolean; reason?: string; patientKey: string } => {
...
export const removeHistoryEntry = async (
  patientKey: string,
  dateKey?: string,
  removeAll?: boolean
): Promise<boolean> => {
