type ConfirmationKind = "confirmed" | "cancelled" | "missed";
...
export const requestStatusConfirmation = async (request: ConfirmationRequest): Promise<boolean> => {
  if (activePrompt) {
    return activePrompt;
  }
  ...
  activePrompt = new Promise<boolean>(resolve => {
    ...
    badge.textContent = request.statusKind === "confirmed"
      ? "Confirmação de presença"
      : request.statusKind === "missed"
        ? "Confirmação de falta"
        : "Confirmação de desmarcação";
    ...
    title.textContent = request.statusKind === "confirmed"
      ...
      : request.statusKind === "missed"
        ? "Confirmar envio desta falta ao chat?"
    ...
    confirmButton.textContent = request.statusKind === "confirmed"
      ? "Confirmar presença e enviar"
      : request.statusKind === "missed"
        ? "Confirmar falta e enviar"
        : "Confirmar desmarcação e enviar";
