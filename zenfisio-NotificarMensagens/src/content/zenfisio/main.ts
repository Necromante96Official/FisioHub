...
const createFloatingStateCard = async (): Promise<void> => {
  ...
  const createMenuButton = (label: string, danger = false): HTMLButtonElement => {
  ...
  const setMenuOpen = async (open: boolean): Promise<void> => {
    menuOpen = open;
  ...
  const refreshStatusBox = async (): Promise<void> => {
    try {
