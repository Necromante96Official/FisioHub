...
import { containsTriggerStatusLabel, isKnownZenfisioStatusLabel, isTriggerStatusLabel } from "../../shared/statusOptions.js";
...
const missedSet = new Set<string>([
  normalizeText(STATUS_LABELS.MISSED_WITH_NOTICE),
  normalizeText(STATUS_LABELS.MISSED_WITHOUT_NOTICE),
  "faltou (com aviso previo)",
  "faltou (sem aviso previo)"
]);
...
const resolveStatusKind = (normalizedLabel: string): StatusKind | null => {
  ...
  if (missedSet.has(normalizedLabel)) {
    return "missed";
  }
  return null;
};
...
const rememberCurrentSelectValue = (event: Event): void => {
...
const handleClickTrigger = async (event: MouseEvent): Promise<void> => {
  if (!event.isTrusted) {
    return;
  }
...
export const initStatusMonitor = (): void => {
