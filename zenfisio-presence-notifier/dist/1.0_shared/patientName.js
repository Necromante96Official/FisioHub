...
export const patientKeyFromName = (name) => normalizeText(name);
export const sanitizePatientName = (rawName) => {
    if (!rawName) {
        return null;
    }
    let text = rawName
...
export const isLikelyValidPatientName = (name) => {
    if (!name) {
        return false;
    }
...
export const extractPatientNameFromText = (text) => {
    if (!text) {
        return null;
    }
    const patterns = [
